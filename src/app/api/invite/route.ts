  ...
  try {
    ...
    if (resendKey) {
      ...
      await resend.emails.send({
        ...
        html: `
          <div style="background-color: #090907; color: #FFFFFF; padding: 40px; font-family: sans-serif;">
            <h1 style="color: #FFFFFF;">ANTIGRAVITY</h1>
            <p>You have been invited to join the team as a <strong>${role}</strong>.</p>
            <a href="${inviteUrl}" style="background-color: #FFFFFF; color: white; padding: 12px 24px; text-decoration: none; border-radius: 4px; display: inline-block; margin-top: 20px;">
              ACCEPT INVITE
            </a>
          </div>
        `,
