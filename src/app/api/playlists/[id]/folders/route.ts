...
interface PlaylistFolderItemRow {
  id?: string;
  playlist_id: string;
  folder_id: string;
}

interface PlaylistFolderRow {
  id: string;
}
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    if (isSupabaseConfigured()) {
      ...
      return NextResponse.json({ folder_ids: ((data ?? []) as Pick<PlaylistFolderItemRow, 'folder_id'>[]).map((r) => r.folder_id) });
    }
    const rows = query<PlaylistFolderItemRow>('playlist_folder_items', (r) => r.playlist_id === id);
    return NextResponse.json({ folder_ids: rows.map((r) => r.folder_id) });
  ...
  try {
    if (isSupabaseConfigured()) {
      ...
      if (wanted.length > 0) {
        ...
        const ownedSet = new Set(((owned ?? []) as PlaylistFolderRow[]).map((f) => f.id));
    ...
    const existing = getAll<PlaylistFolderItemRow>('playlist_folder_items');
    existing.filter((r) => r.playlist_id === id && r.id).forEach((r) => deleteRow('playlist_folder_items', r.id as string));
