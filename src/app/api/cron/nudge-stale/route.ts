    ...
    for (const s of (sends ?? []) as unknown as BeatSendWithContactRow[]) {
      ...
      try {
        const { error: rErr } = await resend.emails.send({
          ...
          html: `
            <div style="font-family: sans-serif; background: #090907; color: #FFFFFF; padding: 40px; border-radius: 20px;">
              <h1 style="text-transform: uppercase; letter-spacing: 0.3em; font-size: 14px; color: #FFFFFF;">${copy.subject}</h1>
