...
const log = createLogger('api.tracks.shares');

export const runtime = 'nodejs';

/**
 * GET  /api/tracks/[id]/shares  → list track shares (owner only)
 * POST /api/tracks/[id]/shares  → create a share token for this track
 *
 * Creates a row in project_shares with content_type='track'.
 * Share resolves to /projects/share/{token} using the full variant experience.
 */
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const { data, error } = await owner.admin
      .from('project_shares')
      .select('id, track_id, token, role, allow_downloads, expires_at, invited_email, label, plays, created_at, revoked_at, recipient_kind, sales_enabled, content_type')
      .eq('track_id', id)
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    if (!isSupabaseConfigured()) {
      return NextResponse.json({ error: 'Track shares require Supabase' }, { status: 501 });
    }
    const owner = await requireRowOwnership('tracks', id);
    if (!owner.ok) return owner.res;
    ...
    const { data, error } = await owner.admin
      ...
      .select('id, track_id, token, role, allow_downloads, expires_at, invited_email, label, plays, created_at, recipient_kind, sales_enabled, content_type')
      .single();
    if (error) throw error;
