    ...
    return (
      <Centered>
        ...
        <div className="flex flex-wrap items-center justify-center gap-2">
          ...
          <Link
            href="/store/account"
            className="flex min-h-11 items-center gap-1.5 rounded-full border border-white/20 px-4 py-2 text-[10px] font-mono uppercase tracking-wider text-white/80 transition-colors hover:border-white/40 hover:text-white"
          >
            <Mail size={11} />
            My account
          </Link>
  ...
  return (
    <Centered>
      <div className="relative mx-auto mb-5 size-16">
        <Loader2 size={64} className="absolute inset-0 animate-spin text-white" />
        <Layers size={20} className="absolute inset-0 m-auto text-white" />
      </div>
      <p className="mb-2 text-[10px] font-mono uppercase tracking-[0.24em] text-white/40">Project delivery</p>
...
function Centered({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-screen bg-[#090907] px-6 py-10 text-center text-white">
      <div className="mx-auto flex min-h-[78vh] max-w-xl items-center justify-center">
        <div className="w-full rounded-[28px] border border-white/10 bg-white/[0.02] px-6 py-10 shadow-[0_30px_90px_rgba(0,0,0,0.38)]">
          {children}
        </div>
      </div>
    </div>
...
export default function ProjectAccessGate() {
  return (
    <Suspense fallback={<Centered><Loader2 size={20} className="animate-spin text-white/40" /></Centered>}>
      <Inner />
    </Suspense>
  );
}
