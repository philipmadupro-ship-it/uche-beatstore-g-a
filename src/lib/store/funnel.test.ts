...
describe('isStoreEventType', () => {
  it('accepts known types, rejects others', () => {
    ...
    expect(isStoreEventType('store_view')).toBe(true);
    expect(isStoreEventType('preview_play')).toBe(true);
...
describe('computeFunnel', () => {
  ...
  it('counts each session once per stage and back-fills shallower stages', () => {
    ...
    const out = computeFunnel([
      { session_id: 's1', event_type: 'store_view' },
      { session_id: 's1', event_type: 'preview_play' },
    ...
    expect(by.store_view).toBe(3); // s1, s2 (back-filled), s3
    expect(by.preview_play).toBe(2); // s1 + purchase back-fill, s2 add back-fill
  ...
  it('computes pctOfTop and pctOfPrev', () => {
    const out = computeFunnel([
      { session_id: 'a', event_type: 'store_view' },
      { session_id: 'b', event_type: 'store_view' },
      { session_id: 'c', event_type: 'store_view' },
      { session_id: 'd', event_type: 'store_view' },
      { session_id: 'a', event_type: 'add_to_cart' },
      { session_id: 'b', event_type: 'add_to_cart' },
      { session_id: 'a', event_type: 'purchase' },
    ]);
    const view = out.find((r) => r.stage === 'store_view')!;
    ...
    expect(cart.pctOfPrev).toBe(100); // both preview-stage sessions reached cart
  ...
  it('maps historical pdp_view events to the storefront entry stage', () => {
    const out = computeFunnel([{ session_id: 'legacy', event_type: 'pdp_view' }]);
    expect(out.find((r) => r.stage === 'store_view')?.sessions).toBe(1);
  });
