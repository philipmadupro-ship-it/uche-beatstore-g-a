    ...
    <div className="flex items-center justify-between gap-4 px-1 py-3 flex-wrap">
      ...
      <div className="flex items-center gap-1">
        <button
          ...
          disabled={page <= 1}
          className="h-8 px-2 rounded-md flex items-center gap-1 text-[11px] text-white/80 hover:text-white hover:bg-white/10 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
        >
          <ChevronLeft size={14} /> Prev
        </button>
        {nums.map((n, i) =>
          n === '…' ? (
            <span key={`gap-${i}`} className="px-1.5 text-[11px] text-white/40">…</span>
          ) : (
            <button
              key={n}
              onClick={() => onPage(n)}
