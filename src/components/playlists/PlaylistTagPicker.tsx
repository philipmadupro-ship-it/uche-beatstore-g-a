...
export function PlaylistTagPicker({ playlistId }: { playlistId: string }) {
  ...
  return (
    <div className="space-y-5 p-4 bg-[#16130e] border border-[#1f1a13] rounded-2xl w-full max-w-sm shadow-2xl">
      <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-[#4a4338]">Playlist tags</h3>
            ...
            {options.map((tag) => {
              ...
              return (
                <button key={tag} onClick={() => handleToggle(tag, category)}
                  className={`px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all border ${active ? 'bg-[#2A2418] text-[#E8D8B8] border-[#8A7A5C]' : 'bg-transparent text-[#4a4338] border-[#2d2620] hover:text-[#a08a6a]'}`}>
                  {tag}
                </button>
              );
            })}
