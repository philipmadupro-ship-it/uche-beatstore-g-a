...
export function PlaylistFilterBar({
  ...
  const filterPanel = (
    <div className="space-y-4">
        ...
        <div key={label}>
          ...
          <div className="flex flex-wrap gap-1.5">
            {opts.map((tag) => {
              ...
              return (
                <button key={tag} onClick={() => toggleTag(tag)} className={`px-2.5 py-1 rounded-full text-[11px] font-medium border transition-all ${active ? 'bg-[#E7D7BE] text-black border-[#E7D7BE]' : 'bg-[#171511] border-[#2B2821] text-[#B4AA99] hover:text-[#D0C3AF] hover:border-[#3B372F]'}`}>
                  {tag}
                </button>
              );
            })}
      ...
      {activeCount > 0 && (
        <button onClick={() => onChange({ ...value, search: '', folder: 'all', tags: new Set() })} className="flex items-center gap-1 text-[10px] font-mono uppercase tracking-wider text-[#B4AA99] hover:text-[#F7EBDD] transition-colors">
          <X size={11} /> Clear all
        </button>
      )}
    </div>
