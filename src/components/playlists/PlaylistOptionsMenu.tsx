          ...
          <>
            ...
            <div onClick={(e) => { e.preventDefault(); e.stopPropagation(); }}
              ...
              {renaming ? (
                ...
                <>
                  <MI icon={<Pin size={13} className={isPinned ? 'text-[#D4BFA0]' : ''} />} label={isPinned ? 'Unpin' : 'Pin to top'} busy={busy === 'pin'} onClick={async () => { await patch({ pinned: !isPinned }, 'pin'); setOpen(false); toast.success(isPinned ? 'Unpinned' : 'Pinned to top'); }} />
...
function MI({ icon, label, onClick, danger, busy }: { icon: React.ReactNode; label: string; onClick: () => void; danger?: boolean; busy?: boolean }) {
  return (
    <button onClick={onClick} disabled={busy} className={`w-full flex items-center gap-2.5 px-3 py-2 text-[12px] font-medium transition-colors disabled:opacity-50 ${danger ? 'text-red-400 hover:bg-red-500/10' : 'text-[#E8DCC8] hover:bg-[#16130e]'}`}>
