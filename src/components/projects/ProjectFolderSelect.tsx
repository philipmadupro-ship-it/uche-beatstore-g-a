      ...
      <div className="w-full max-w-sm rounded-2xl border border-[#1f1a13] bg-[#0e0c08] shadow-2xl flex flex-col max-h-[80vh]" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#1a160f]">
          <div className="flex items-center gap-2">
            <FolderPlus size={13} className="text-[#a08a6a]" />
            <h3 className="text-[12px] font-bold text-[#E8DCC8]">Move to folders</h3>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-3 py-3 space-y-1">
          {loading ? (
            <div className="flex items-center justify-center py-8"><Loader2 size={16} className="animate-spin text-[#4a4338]" /></div>
          ) : folders.length === 0 ? (
            <p className="text-[11px] text-[#5a5142] text-center py-6">No folders yet — create one below.</p>
          ) : (
            folders.map((f) => {
              const on = selected.has(f.id);
              return (
                <button key={f.id} onClick={() => toggle(f.id)}
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left transition-colors ${on ? 'bg-[#2A2418]' : 'hover:bg-[#14110d]'}`}>
                  <span className={`w-4 h-4 rounded flex items-center justify-center border shrink-0 ${on ? 'bg-[#D4BFA0] border-[#E8D8B8]' : 'border-[#2d2620]'}`}>
        ...
        <div className="px-3 py-3 border-t border-[#1a160f] flex items-center gap-2">
          ...
          <button onClick={createFolder} disabled={!newName.trim() || creating}
            className="shrink-0 flex items-center gap-1 px-3 py-2 rounded-lg bg-[#14110d] border border-[#1f1a13] text-[11px] font-medium text-[#a08a6a] hover:text-[#E8DCC8] hover:border-[#2d2620] disabled:opacity-40 transition-colors">
            {creating ? <Loader2 size={12} className="animate-spin" /> : <Plus size={12} />}
          </button>
        </div>

        <div className="px-5 py-3 border-t border-[#1a160f] flex items-center justify-end gap-2">
          <button onClick={onClose} className="px-3 py-2 rounded-lg text-[11px] font-mono uppercase tracking-wider text-[#6a5d4a] hover:text-[#E8DCC8] transition-colors">Cancel</button>
          <button onClick={save} disabled={saving}
            className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-[11px] font-bold uppercase tracking-wider bg-[#D4BFA0] text-black hover:bg-[#E8D8B8] transition-colors disabled:opacity-40">
