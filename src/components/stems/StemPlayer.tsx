...
export function StemPlayer({ vocalsUrl, drumsUrl, bassUrl, otherUrl }: StemPlayerProps) {
  ...
  const StemRow = ({ name, iconColor }: { name: string, iconColor: string }) => (
    <div className="flex flex-col gap-3 p-6 bg-[#090907] border border-white/10 rounded-2xl">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <button onClick={() => toggleMute(name.toLowerCase())} className={`${mutes[name.toLowerCase() as keyof typeof mutes] ? 'text-red-500' : 'text-white/40 hover:text-white'} transition-colors`}>
