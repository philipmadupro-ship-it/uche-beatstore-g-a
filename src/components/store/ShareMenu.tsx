...
export function ShareMenu({
  trackId, trackTitle, producerName, accentColor = '#D4BFA0',
}: Props) {
    ...
    <>
      <div className="relative shrink-0" ref={wrapRef}>
        <button
          ...
          aria-expanded={open}
          title="Share"
          className="w-10 h-10 rounded-full bg-white/[0.04] border border-white/[0.08] flex items-center justify-center text-[#a08a6a] hover:text-[#E8DCC8] hover:bg-white/[0.08] hover:border-white/[0.16] transition-colors"
        >
          <Share2 size={14} />
        </button>
        ...
        {open && (
          <div
            role="menu"
            className="absolute right-0 top-12 z-30 w-60 rounded-xl bg-[#14110d]/95 backdrop-blur-xl border border-white/[0.10] shadow-[0_24px_60px_rgba(0,0,0,0.6)] py-1.5"
          >
            <button
              role="menuitem"
              onClick={() => { setOpen(false); copyLink(); }}
              className="flex items-center gap-2 w-full px-3 py-2.5 text-[12px] text-[#E8DCC8] hover:bg-white/[0.06] transition-colors text-left"
            >
            ...
            <Link
              ...
              className="flex items-center gap-2 px-3 py-2.5 text-[12px] text-[#E8DCC8] hover:bg-white/[0.06] transition-colors"
            >
