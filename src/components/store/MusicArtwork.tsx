  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Center label */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[36%] h-[36%] rounded-full overflow-hidden border border-white/20 bg-white/[0.04] flex items-center justify-center">
          {albumArt ? (
            <img src={albumArt} alt="" className="w-full h-full object-cover" />
          ) : (
            <MusicIcon size={16} className="text-white/30" />
          )}
          {/* Center spindle hole */}
      ...
      {showTip && music && (
        <div className="pointer-events-none absolute left-2 bottom-2 px-2 py-1 rounded bg-black/75 backdrop-blur text-[9px] font-mono uppercase tracking-[0.15em] text-white z-20 max-w-[80%] truncate">
          {music}
        </div>
      )}
    </div>
