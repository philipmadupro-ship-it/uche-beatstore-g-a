...
export function ShareCardButton({
  ...
  const nativeShare = async () => {
    if (typeof navigator === 'undefined' || !navigator.share) {
      // No native share — fall back to opening the image in a new tab
      window.open(cardUrl, '_blank');
      return;
    }
    setSharing(true);
    try {
      // Fetch the card as a blob so we can share it as a real file
      // (more share targets accept Files than URLs).
      ...
      if (navigator.canShare?.({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: trackTitle,
          text: `Now playing: ${trackTitle}`,
        });
      } else {
        // Fallback: share the URL instead.
        await navigator.share({
          title: trackTitle,
          text: `Check this beat: ${trackTitle}`,
          url: window.location.href,
        });
      }
    } catch (err) {
      // User dismissed the share sheet, or share-files aren't supported.
      // Don't toast that as an error — it's normal.
