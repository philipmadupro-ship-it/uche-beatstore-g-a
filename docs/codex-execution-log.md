...
## Upload crash fix + AI cover generation

### 1. The upload crash — reproduced, root-caused, fixed

Reported as "crashing when I was using it yesterday", no error text. Drove the real UI to find
it: a 258 KB WAV (single part) and a 6.7 MB WAV (true multipart) both uploaded cleanly, so the
happy path was not the problem.

The crash is on **restore**, not upload. `hydrate()` did
`JSON.parse(localStorage) as PersistedItem[]` and then read `p.completedPartNumbers.length`
straight off it. One entry missing that field throws.

That throw did far more damage than a broken tray. `UploadsTray` is mounted in
`DashboardGroupLayout`, so the exception propagated out of the layout and **took down every
dashboard route at once** — library, projects, sales, settings. And because the offending value
stayed in localStorage, it reproduced on every reload: an unrecoverable dashboard for anyone who
would not think to clear their site data by hand. Reproduced in-browser (screenshot of the
Next.js overlay pointing at `manager.ts:446`) before writing a line of the fix.

**Fix, two layers.** `lib/upload/persisted-uploads.ts` treats localStorage as the untrusted
input it is: normalise every field, salvage coherent rows, drop the rest, never throw. 13 tests,
including the exact crashing payload and a sweep of garbage inputs. Then
`WidgetErrorBoundary` — the app had **no error boundary anywhere** — around each always-on
widget in the dashboard layout, so no future fault in an accessory widget can blank the page
the producer is working on. `children` is deliberately left unwrapped: a genuine page error
should still surface.

Verified with the same localStorage payload that white-screened the dashboard: it now renders
fully and shows the salvaged row as "interrupted — re-pick the same file to resume from 0%".

### 2. Cover generator

The Cover Art Studio already had layers, an 8-colour palette and four art directions. Two
things were missing.

**The image layer was dead.** `ImageArtworkLayer.src` existed on the type but
`renderArtworkDocumentSvg` never read it — every image layer drew as the grey placeholder box
regardless. Any generated or uploaded artwork would have been invisible. Now renders a real
`<image>` with `xMidYMid slice`, plus SVG filters so the existing `duotone` /
`mineral-tint` / `high-contrast` treatments actually apply. 6 tests, including attribute-escape
on a crafted URL.

**AI generation** — `lib/cover/image-generation.ts` (pure: prompt building, provider
resolution, response parsing; 25 tests), `POST /api/cover/generate` (auth-gated — it spends real
money per call), and the `Media` tool panel, which was an unimplemented placeholder.

Design decisions worth recording:
- **Keys are server-side env vars only, never accepted from the browser.** A key posted from the
  client shows up in the network panel, in every proxy and error log on the way, and in this
  app's own request logging — and would sit in localStorage for any XSS to read. `GET` on the
  route returns availability booleans, never a key or a prefix.
- **Requesting an unconfigured provider errors rather than falling back.** Silently billing the
  other provider, and returning a different house style, is worse than failing.
- **"No text" is always appended to the prompt.** The studio composes titles as real text
  layers; generated lettering is malformed and would collide with the real title.
- **Generated bytes are inlined as a data URI for the layer.** The image lives on R2
  (cross-origin) and the SVG export path renders through `<img>`, which cannot fetch
  cross-origin — a bare URL would export a cover with the artwork silently missing while the
  on-screen preview looked right. The R2 URL is kept for attaching as the actual cover.

### Verified

Panel renders in the studio: provider warning with exact setup instructions, prompt box with six
style presets, colour-influence chips bound to the live palette, avoid field, correctly disabled
Generate button, empty state.

**Not verified: an actual generation round-trip.** No provider key is configured in this
environment and I did not ask for one. Prompt construction and both providers' response parsing
are unit-tested against captured shapes, and the request/error paths are written against the
documented APIs, but the first real call to OpenAI or Gemini has not been made.

`tsc` clean · `eslint` 0 errors · **674 tests** (was 630) · build green.
