...
- **Both editors can take the whole window** (`hooks/useEditorExpand`, `ui/EditorViewButtons`): Expand is `fixed inset-0 z-[100]` — over the TopBar/PlayerBar (z-50), under popovers (200) — and Full screen then asks the browser to hide its own chrome, on `document.documentElement`, never the editor, because every menu and toast portals to `<body>` and would vanish behind a fullscreen sub-element. Keep `relative` out of the class list while expanded: beside `fixed`, Tailwind emits `relative` later and it wins, so the "overlay" silently stays in the page flow.
- **Zoom is `ui/ZoomControl` + `lib/ui/zoom.ts`** in both editors, floating at the bottom of the canvas: log-scale slider, a stop ladder for −/+, click the percentage for 100%. The cover art slider used to stop at 60%.
