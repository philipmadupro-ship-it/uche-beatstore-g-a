...
- **Every `/api/*` route is producer-only for signed-in callers unless listed in `lib/security/api-gate.ts`.** Buyers sign in through the same Supabase auth, so `requireUser()` alone let any buyer send email from the verified domain, write to R2 and spend AI credits. `src/proxy.ts` now 403s a signed-in user without a `creator_profiles` row on any non-allowlisted API path; signed-out calls fall through to the route (401 / cron bearer / webhook signature). A new buyer- or share-facing route must be added to the allowlist, and must do its own authorisation.
